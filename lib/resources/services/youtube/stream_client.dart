...
class AudioStreamClient {
  ...
  Stream<List<int>> _getStream(
    ...
    final int downloadChunkSize = isThrottledOrVeryLarge
        ? 10379935
        : (end -
            start); //10mb is the size, lets try 5mb later to see if download speed is better or not

    while (currentChunkStart < end) {
      final currentChunkEnd =
          math.min(currentChunkStart + downloadChunkSize, end);
      ...
      try {
        final response = await retry(this, () async {
          ...
          if (url.queryParameters['c'] == 'ANDROID' ||
              url.host.contains('googlevideo.com')) {
        ...
        if (bytesReceivedForThisChunk <
                (currentChunkEnd -
                    (currentChunkStart - bytesReceivedForThisChunk)) &&
            currentChunkStart < end) {
          if (bytesReceivedForThisChunk == 0 && response.statusCode == 206) {
          } else if (bytesReceivedForThisChunk == 0 &&
              response.statusCode != 200 &&
              response.statusCode != 206) {
            throw Exception(
                'Chunk download received no data with status: ${response.statusCode}');
          }
        }
  ...
  void _validateResponse(http.BaseResponse response, int statusCode) {
    ...
    if (statusCode == 416) {
      AppLogger.debug(
          'Warning: Received 416 Range Not Satisfiable for ${request.headers['Range']}');
    }
  }
